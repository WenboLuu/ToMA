...
from .merge import merge_helper, do_nothing


def compute_merge(
    x: torch.Tensor, key_word, rope_emb, toma_info: Dict[str, Any]
) -> Tuple[Callable, ...]:
    args = toma_info["args"]
    ...
    if args["ratio"] > 0:
        ...
        m, u = merge_helper(
            x,
            r,
            num_of_tiles=args["num_tiles"],
            dst_selection=args["dst_selection"],
            merge_scheduler=args["merge_scheduler"],
            key_word=key_word,
            rope_emb=rope_emb,
    ...
    def prepare_attention_mask(
        ...
        if current_length != target_length:
            if attention_mask.device.type == "mps":
                ...
                # TODO: re-enable tests/models/test_models_merge_2d_condition.py#test_model_xattn_padding
...
class FluxGlobalAttentionProcessor:
    ...
    def __call__(
        ...
        clean_hidden_states = hidden_states

        image_rotary_emb = torch.stack(image_rotary_emb, dim=0)

        from .utils import apply_rotary_emb

        inner_dim = clean_hidden_states.shape[-1]
        head_dim = inner_dim // attn.heads

        merge, unmerge = compute_merge(
            clean_hidden_states, "image", image_rotary_emb[:, 512:, :], self._toma_info
        )
        if merge == False:
            batch_size, _, _ = (
                hidden_states.shape
            ...
            merged_clean_hidden_states, dst_idx, image_rotary_emb_merged = merge(
                clean_hidden_states
            )

            query = attn.to_q(merged_clean_hidden_states)
            clean_key = attn.to_k(merged_clean_hidden_states)
            clean_value = attn.to_v(merged_clean_hidden_states)

            inner_dim = clean_key.shape[-1]
            head_dim = inner_dim // attn.heads

            query = query.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)
            clean_key = clean_key.view(batch_size, -1, attn.heads, head_dim).transpose(
                1, 2
            )
            clean_value = clean_value.view(
                batch_size, -1, attn.heads, head_dim
            ).transpose(1, 2)
            ...
            if attn.norm_q is not None:
                query = attn.norm_q(query)
            if attn.norm_k is not None:
                clean_key = attn.norm_k(clean_key)
            ...
            if encoder_hidden_states is not None:
                ...
                query = torch.cat([encoder_hidden_states_query_proj, query], dim=2)
                key = torch.cat([encoder_hidden_states_key_proj, clean_key], dim=2)
                value = torch.cat(
                    [encoder_hidden_states_value_proj, clean_value], dim=2
                )

                image_rotary_emb_for_prompt = image_rotary_emb[:, :512, :]

            if image_rotary_emb is not None:
                image_rotary_emb_merged = torch.cat(
                    [image_rotary_emb_for_prompt, image_rotary_emb_merged], dim=1
                )

                query = apply_rotary_emb(query, image_rotary_emb_merged)
                key = apply_rotary_emb(key, image_rotary_emb_merged)
            ...
            image_hidden_states = unmerge(image_hidden_states)
...
class FluxStripeWiseAttentionProcessor:
    """Attention processor used typically in processing the SD3-like self-attention projections."""
    ...
    def __call__(
        ...
        if encoder_hidden_states is not None:
            len_prompt = encoder_hidden_states.size(-2)
            len_hidden = hidden_states.size(-2)
        else:
            len_prompt = 0
        ...
        if merge == False:
            ...
            query = attn.to_q(clean_hidden_states)
            clean_key = attn.to_k(merged_clean_hidden_states)
            clean_value = attn.to_v(merged_clean_hidden_states)
